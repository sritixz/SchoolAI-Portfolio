...
import { teacherEngagementData } from "../../data/schoolAdminData";
        ...
        <div className="flex items-center gap-3 mb-4 flex-wrap">
          ...
          <div className="flex items-center gap-2 ml-2">
            ...
            <div className="flex gap-1">
              <div className="size-4 bg-green-400 rounded-full" />
              <div className="size-4 bg-yellow-400 rounded-full" />
              <div className="size-4 bg-red-400 rounded-full" />
            </div>
          </div>
          <select className="ml-auto text-xs border border-gray-200 rounded-lg px-2 py-1.5">
            ...
            <div className="bg-white rounded-xl border border-gray-100 shadow-sm p-4 mb-4">
              <div className="grid grid-cols-4 gap-4 items-center">
                <div>
                  ...
                  <p className="text-2xl font-black text-[#100e1a]">{d.avgEngagement}<span className="text-sm font-normal text-gray-400">/100</span></p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Active Teachers</p>
                  <p className="text-2xl font-black text-[#100e1a]">{d.activeTeachers}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500">Support Opps</p>
                  <p className="text-2xl font-black text-[#695be6]">{String(d.supportOpps).padStart(2, "0")}</p>
                </div>
                <div>
                  <p className="text-xs text-gray-500 mb-1">Engagement Distribution</p>
                  <p className="text-[10px] text-gray-400 mb-1">Breakdown by status level</p>
                  <svg viewBox="0 0 36 36" className="size-12">
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke="#e5e7eb" strokeWidth="4" />
                    <circle cx="18" cy="18" r="15.9" fill="none" stroke="#695be6" strokeWidth="4"
                ...
                return (
                  <div key={t.name} className={`bg-white rounded-xl border ${cfg.bg} shadow-sm p-4`}>
                    ...
                    <div className="grid grid-cols-3 gap-2 text-center mb-3">
                      <div>
                        <p className="text-[9px] text-gray-400">HW</p>
                        <p className="text-sm font-black">{t.hw}%</p>
                      </div>
                      <div>
                      ...
                      <div>
                        <p className="text-[9px] text-gray-400">INTRV</p>
                        <p className="text-sm font-black">{String(t.intrv).padStart(2, "0")}</p>
                      </div>
                    </div>
              ...
              {d.suggestedPD.map((pd, i) => (
                <div key={i} className="border border-gray-100 rounded-xl p-3 mb-3">
                  ...
                  <div className="flex justify-end mt-1">
                    <span className="material-symbols-outlined text-[#695be6] text-base cursor-pointer">
                      {i === 1 ? "play_circle" : i === 2 ? "download" : "arrow_forward"}
                    </span>
                  </div>
                </div>
              ))}
